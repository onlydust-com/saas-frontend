import { Switch as HeadlessSwitch } from "@headlessui/react";
import { FocusEventHandler, Ref, forwardRef } from "react";

import { cn } from "@/shared/utils";

import { Field, FieldProps } from "./Field";

export interface FieldSwitchProps extends Omit<FieldProps, "children"> {
  switchLabel?: string;
  className?: string;
  value?: boolean;
  onChange?: (value: boolean) => void;
  onFocus?: FocusEventHandler<HTMLButtonElement>;
  onBlur?: FocusEventHandler<HTMLButtonElement>;
}

export const FieldSwitch = forwardRef(function FieldSwitch(
  { onBlur, switchLabel, onFocus, onChange, className, value, ...rest }: FieldSwitchProps,
  ref: Ref<HTMLButtonElement>
) {
  return (
    <Field {...rest}>
      <HeadlessSwitch.Group>
        <div className="flex items-center gap-3">
          <HeadlessSwitch
            checked={value || false}
            onChange={onChange}
            onBlur={onBlur}
            onFocus={onFocus}
            ref={ref}
            className={cn(
              "relative inline-flex h-6 w-11 items-center rounded-full",
              value ? "bg-spacePurple-500" : "bg-white/8",
              className
            )}
          >
            <span
              className={cn(
                "bg-greyscale-50 inline-block h-4 w-4 transform rounded-full transition",
                value ? "translate-x-6" : "translate-x-1"
              )}
            />
          </HeadlessSwitch>
          {switchLabel && (
            <HeadlessSwitch.Label className="text-spaceBlue-200 text-sm">{switchLabel}</HeadlessSwitch.Label>
          )}
        </div>
      </HeadlessSwitch.Group>
    </Field>
  );
});
